...
public class App extends BasicApp {
    ...
    public void start() {
        ...
        var endpointDataGateway = new EndpointDataGateway();
        var finder = new EndpointWorkFinder(endpointDataGateway);
        var worker = new EndpointWorker(restTemplate, articleDataGateway);
        var scheduler = new WorkScheduler<>(finder, List.of(worker));
